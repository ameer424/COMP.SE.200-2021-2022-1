...
describe('Difference function', function(){
    ...
    it('difference([1, 2, 3, 3, 3],[3]) should be equal to [1, 2]',function(){
        assert.isArray(difference([1, 2, 3, 3, 3], [3]));
        assert.sameMembers(difference([1, 2, 3, 3, 3], [3]), [1, 2]);
    });

    it('difference([ ],[5, 1, 2]) should be equal to [ ]',function(){
        assert.isArray(difference([], [5, 1, 2]));
        assert.sameMembers(difference([], [5, 1, 2]), []);
    });

    it('difference([5, 1, 2],[ ]) should be equal to [5, 1, 2]',function(){
        assert.isArray(difference([5, 1, ], []));
        assert.sameMembers(difference([], [5, 1, 2]), []);
    });
});